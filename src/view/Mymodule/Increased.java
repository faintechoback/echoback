...
public class Increased extends JDialog { // 模态框在被手动关闭前，用户只能操作模态框内的内容
    public Increased() {
        ...
        Object []value = new Object[length];// 提取值后将数据都放进enter,为了适应各种类型参数，采取object型
        ...
        for (int n = 1; n < length; n++) {// 将n设为1，排去对数据无太多意义的序号字段，但在sql语句中需要进行一定的修改
        ...
        this.add(submit);
        //对于部分可以为空和不可以为空的元素无法直接检测
        // 基本的属性设置，大小需要进行调节，或许也可以通过行数来设置不同的高度
        this.setSize(new Dimension(length*70, 60 * length));
        this.setTitle("添加信息");
        setLayout(new GridLayout(length + 1, 1));// 一个输入框一行，即一个元素占一行
